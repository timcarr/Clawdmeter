...
- **Adaptive poll cadence.** The daemon polls every **45 s while your usage is rising** and drops to **60 s when idle**, so the meter feels real-time exactly when you're using Claude. (Measured: the endpoint reports whole percents that move about once a minute under heavy use, and its rate limiter allows ~1.6 requests/min sustained — so 45 s catches every step while staying under the limit.) If the endpoint ever rate-limits (HTTP 429), the daemon holds the 60 s cadence until 5 minutes after the last 429, then tries fast polling again.
...
2. It GETs `api.anthropic.com/api/oauth/usage` — the same endpoint Claude Code's `/usage` command uses. It consumes no tokens, so keeping the meter alive costs nothing. Cadence is adaptive: every 45 s while usage is rising, every 60 s when idle, and pinned to 60 s until 5 minutes after the last HTTP 429 if the endpoint rate-limits.
